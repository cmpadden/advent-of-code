...
Another push of the button leaves you in the familiar hallways of some friendly amphipods! Good
thing you each somehow got your own personal mini submarine. The Historians jet away in search of
...
qo, a disk map like 12345 would represent a one-block file, two blocks of free space, a three-block
 ...
 * 0 = 0, 1 * 0 = 0, 2 * 9 = 18, 3 * 9 = 27, 4 * 8 = 32, and so on. In this example, the checksum
...
pub fn p1(input: &str) -> usize {
    let disk_map: Vec<i32> = input
        .replace("\n", "")
        .chars()
        .map(|x| x.to_string().parse::<i32>().unwrap())
        .collect();

    fn to_filesystem(map: Vec<i32>) -> Vec<Block> {
        let mut filesystem: Vec<Block> = Vec::new();
        for (id, map) in map.iter().enumerate() {
            for _ in 0..*map {
    ...
    let mut fs = to_filesystem(disk_map);
    ...
    while p1 < p2 {
        println!("{:?}", (p1, p2));
        if fs[p1].empty && fs[p2].empty {
            p2 -= 1;
        } else if fs[p1].empty && !fs[p2].empty {
            fs.swap(p1, p2);
            p1 += 1;
            p2 -= 1;
        } else if !fs[p1].empty && fs[p2].empty {
...
mod tests {
    ...
    #[test]
    fn test_p1p2() {
        assert_eq!(p1("2333133121414131402"), 1928);
