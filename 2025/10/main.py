...
for L in open("2025/10/input.txt"):
    t = [c == "#" for c in re.search(r"\[(.*?)\]", L).group(1)]
    n = len(t)
    B = [
        [i in map(int, m.group(1).split(",")) for i in range(n)]
        for m in re.finditer(r"\(([0-9,]+)\)", L)
    ]
    m = len(B)
    A = [[int(B[j][i]) for j in range(m)] + [int(t[i])] for i in range(n)]
    P = []
    r = 0
    for c in range(m):
        ...
        P.append(c)
        r += 1
    F = [i for i in range(m) if i not in P]
    M = 9**9
...
S2 = []
for L in open("2025/10/input.txt"):
    targets = list(map(int, re.search(r"\{(.*?)\}", L).group(1).split(",")))
    n = len(targets)
    B = [
        [i in map(int, m.group(1).split(",")) for i in range(n)]
        for m in re.finditer(r"\(([0-9,]+)\)", L)
    ...
    m = len(B)
    A = [
        [Fraction(int(B[j][i])) for j in range(m)] + [Fraction(targets[i])]
        for i in range(n)
    ]
    P = []
    r = 0
    ...
    if len(F) == 0:
        ...
        for i, c in enumerate(P):
            x[c] = A[i][m]
        if all(v >= 0 and v.denominator == 1 for v in x):
            M = sum(int(v) for v in x)
    elif len(F) <= 2:
        for free_vals in product(range(max(targets) + 1), repeat=len(F)):
            x = [Fraction(0)] * m
            for i, v in enumerate(F):
                x[v] = Fraction(free_vals[i])
            valid = True
            for i, c in enumerate(P):
                val = A[i][m]
                for j in range(m):
                ...
                x[c] = val
            if valid:
                M = min(M, sum(int(v) for v in x))
    else:
        for free_vals in product(range(201), repeat=len(F)):
            x = [Fraction(0)] * m
            for i, v in enumerate(F):
                x[v] = Fraction(free_vals[i])
            valid = True
