...
# For distinct values in hand, try replacing J with that value and see the strongest
# value. Corner case -- if there are multiple Js, replacing them with the same value may
# not be the strongest hand.

hand = 'T55J5'

s = list(set(hand))

def strongest_hand_with_js(hand):
    s = list(set(hand))
    if 'J' in hand:
        if len(s) == 1:  # all Js
            return 1
        return min([strength(hand.replace('J', c)) for c in [c for c in s if c != 'J']])
    else:
...
hand_ranks = [
    (strongest_hand_with_js(hand), hand.translate(strength_trans), value)
    for (hand, value) in
    [l.split(' ') for l in lines]
]
