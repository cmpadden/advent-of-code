...
def _arrange_lenses(_input):
    boxes = { x: OrderedDict() for x in range(256) }
    for op in _input.split(','):
        key, operation, focal_length = _parse(op)
        box_num = _hash(key)
        if operation == '=':
            boxes[box_num][key] = focal_length
        if operation == '-':
            if key in boxes[box_num]:
                del boxes[box_num][key]
    return boxes
...
def _tally(bs):
    t = 0
    for k, v in bs.items():
        for n, fl in enumerate(v.values()):
            t += (k + 1) * (n + 1) * int(fl)
    return t

ex = "rn=1,cm-,qp=3,cm=2,qp-,pc=4,ot=9,ab=5,pc-,pc=6,ot=7"

assert _tally(_arrange_lenses(ex)) == 145

_in = open('input.txt').read().strip()

_tally(_arrange_lenses(_in))
